...
- `ticks`、`kbars` 設計用途為盤後分析與回測，並非即時行情來源，盤中請勿反覆輪詢；超過次數限制或造成系統負載過高，將依違規處置暫停使用權。盤中需要即時資料，請改用[行情訂閱](../streaming/stocks/)。
...
組合合約（建法見[商品合約](../../contract/#combo)）可直接查詢歷史 `Ticks` 與 `Kbars`，價格欄位皆為組合淨價。
